...
class Moxie:
    ...
    def mount(self, path: str, app: Any) -> None:
        if isinstance(app, Router):
            ...
            # ASGI mounting logic could be added here
            pass
    ...
    async def handle_http(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            # Use DI resolver to get handler arguments
            # We inject BackgroundTasks and Request automatically
            kwargs = await self.di_resolver.resolve_handler_deps(route.handler, request, path_params)
            
            # Manual check for background tasks in signature
